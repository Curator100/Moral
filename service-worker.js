...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title, {
      ...
      icon: '/icon-192.png',
      badge: '/icon-72.png',
      tag: 'tojm-chat-' + Date.now() + '-' + Math.random().toString(36).slice(2), // unique per push so each shows as its own notification with sound
      renotify: true, // if a tag collision ever does happen, still alert (sound/vibrate) instead of silently updating
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      // WindowClient has no display-mode flag, so a plain same-origin
      // Chrome tab used to pass this check just as easily as the installed
      // PWA and won client.focus() first — openWindow (which is what
      // actually launches the installed app) never even ran (confirmed
      // live: tapping the notification with a Chrome tab open just
      // refocused that tab). Distinguish by client.url instead: the
      // installed app's start_url in manifest.json ("/?source=pwa") must
      // be a marker no plain browser tab's URL ever carries.
      for (const client of clientList) {
        if (client.url.indexOf('source=pwa') !== -1 && 'focus' in client) {
